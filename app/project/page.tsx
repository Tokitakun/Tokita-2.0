...
function calculateGap(width: number) {
  // Jarak dibuat lebih lega agar kartu tidak terlalu menumpuk
  const minWidth = 768;
  const maxWidth = 1456;
  const minGap = 110; 
  const maxGap = 180; 
...
export const ProjectShowcase = () => {
  ...
  function getImageStyle(index: number): React.CSSProperties {
    ...
    const isLeft =
      (activeIndex - 1 + projects.length) % projects.length === index;

    const isRight =
      (activeIndex + 1) % projects.length === index;
    ...
    if (isActive) {
      return {
        ...
        filter:
          "brightness(1) drop-shadow(0 25px 40px rgba(0,0,0,0.45))",
        transition:
    ...
    if (isLeft) {
      return {
        ...
        opacity: 0.28,
        transform:
          `translateX(-${gap}px) translateY(35px) scale(0.72) rotateY(8deg)`,
        filter:
          "brightness(0.5) grayscale(0.9) blur(0.3px)",
        transition:
          "all 0.7s cubic-bezier(0.2, 0.8, 0.2, 1)",
      };
    }
    ...
    return {
      ...
      pointerEvents: "none",
      transform:
        "translateX(0px) translateY(50px) scale(0.65)",
      transition:
        "all 0.7s ease",
    };
  }
        ...
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-8 lg:gap-16 items-center min-h-[600px]">
          ...
          <div className="relative h-[400px] md:h-[500px] w-full flex items-center justify-center perspective-1000">
            ...
            <div className="absolute bottom-0 left-0 w-full flex justify-center gap-4 md:hidden mt-8">
              ...
              <button
                onClick={handlePrev}
                className="p-3 rounded-full bg-[#2A2A2A] border border-white/10 text-white"
              >
                <FaArrowLeft />
              </button>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {activeProject.tags.map((tag, i) => (
                      ...
                      <span
                        key={i}
                        className="px-3 py-1.5 bg-[#252525] border border-white/5 rounded-lg text-xs text-gray-300 font-mono hover:border-[#6B9FBF]/50 hover:text-[#6B9FBF] transition-colors cursor-default"
                      >
                        {tag}
                      </span>
...
// Perubahan visual utamanya cuma di carousel:

// * **Active:** `scale(1)` → tetap besar dan jelas.
// * **Samping:** `scale(0.72)` → lebih kecil.
// * Jarak samping **60–120px → 110–180px**.
// * Opacity samping **0.5 → 0.28**.
// * Samping dibuat sedikit lebih gelap/grayscale.
// * `aspect-video` tetap supaya screenshot website utuh.
// * `object-contain` tetap supaya **nggak ada bagian screenshot yang kepotong**.
// * Kartu yang bukan tetangga langsung dibuat invisible.

// Hasilnya harus lebih seperti **1 screenshot utama + dua preview samar di belakang**, bukan tiga screenshot yang saling bertabrakan.
